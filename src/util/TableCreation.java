...
public class TableCreation {
	public TableCreation() {
		...
		DBRequest.createTable("projects",
				"id SERIAL PRIMARY KEY, name VARCHAR(255) UNIQUE, profit_margin FLOAT, total_cost FLOAT, surface FLOAT, client_id INT references clients(id) ON DELETE CASCADE, status VARCHAR(50)",
				"");

		// Create Components table
		DBRequest.createTable("components",
				"id SERIAL PRIMARY KEY, name VARCHAR(255), unit_cost FLOAT, quantity FLOAT, type VARCHAR(255), tax_rate FLOAT, project_id INT references projects(id) ON DELETE CASCADE",
				"");

		// Create Quotes table
		DBRequest.createTable("quotes",
				"id SERIAL PRIMARY KEY, project_id INT, estimated_amount float, issue_date DATE, validity_date DATE, accepted BOOLEAN DEFAULT false, FOREIGN KEY (project_id) REFERENCES projects(id)",
				"");
